...
#___________________________________________
            #poss_id = pd.read_csv(f'/root/tradesHistory/{name}Trades.csv')
            #pos_id = poss_id['position_id'][len(poss_id)-1]
            response_close = robot.close_market(market=name, position_id='1')
            bot.send_message(75842418, response_close)

        except :
            bot.send_message(75842418, "sell pos not work!")
...
#_________________________________________________
            ...
            position_id = response_market.json()["data"]["position_id"]
            bot.send_message(75842418, f'{name}:{position_id}')
            #bot.send_message(75842418, response_market)
        
            # date, name, position_id, price, position
            dicti = {'date': date[name][len(date) - 1],
                     'name':name,
                     'position_id':position_id,
                     'price':price[name][len(price)-1],
                     'position':'Long' }
             #f'/root/tradesHistory/{name}Trades.csv'
            with open(f'/root/tradesHistory/{name}Trades.csv', 'a', newline='') as csv_file:

                writerObj = csv.writer(csv_file)
                writerObj.writerow(dicti)
...
#bot.send_message(75842418, 'dan dan dan dan\nkahbesin')
#bot.send_message(6001006306, 'dan dan dan dan\nkahbesin')
